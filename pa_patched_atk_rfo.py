...
NEW: Checkbox to exclude TOR from LPG national stock calculation
"""
...
PRODUCT_MAP = {"AGO": "GASOIL", "PMS": "PREMIUM", "LPG": "LPG", "ATK": "ATK", "RFO": "RFO", "AVIATION": "ATK"}
...
def _run_national_analysis(
        ...
        if include_vessels:
            ...
            if _vdf is not None and not _vdf.empty:
                ...
                if not _pending.empty:
                    ...
                    _n_vp = len(_pending)
                    st.write(
                        f'🚢 **Vessel pipeline added** ({_n_vp} pending vessels): '
                        + ' | '.join([f'{p}: **{v:,.0f} LT**'
    ...
    with st.status("🚚 Step 2 / 2 — Fetching national OMC loadings (chunked by week)…", expanded=True) as status_b:
        ...
        if omc_df.empty:
            ...
            filtered_omc = omc_df[omc_df['Product'].isin(['PREMIUM', 'GASOIL', 'LPG'] + extra_products)].copy()
...
def _display_national_results(period_days_arg: int):
    ...
    if include_vessels_res and vessel_pipeline:
        ...
        st.info(
            '🚢 **Vessel pipeline included** — pending cargo added to BDC stock:  '
            + ' | '.join(vessel_parts)
        )
        ...
        with col:
            ...
            <div style='background:rgba(10,14,39,0.85); padding:24px 16px; border-radius:16px;
                ...
                <div style='margin:16px 0 8px;'>
                    ...
                    <div style='color:#888; font-size:14px; margin-top:4px;'>{weeks_text}</div>
